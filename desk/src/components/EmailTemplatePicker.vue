<template>
  ...
  >
    <div class="template-picker__header">
      <div class="template-picker__title">Email Template</div>
      <button
        type="button"
    ...
    <ul v-else class="template-picker__list" role="listbox">
      ...
      >
        <div class="template-picker__row">
          <span class="template-picker__item-title">{{ tpl.name }}</span>
        </div>
    ...
    <div class="template-picker__footer">
      <span class="template-picker__hint"
        >Loads the subject + body; merge fields render per recipient on
        send.</span
      >
      <a
        class="template-picker__manage"
        href="/app/email-template"
        target="_blank"
        rel="noopener"
        >Manage →</a
      >
...
interface Emits {
  (event: "close"): void;
  (event: "select", payload: Selected): void;
}
...
// Raw fetch helper — works in both desk/ and unity_helpdesk/ builds.
...
async function selectTemplate(tpl: Template) {
  try {
    const content = await callMethod<{
      subject: string;
      body: string;
      use_html: boolean;
    ...
    console.warn("[email-template-picker] load content failed:", err);
  }
}
